...
```{r}
cols <- c("Below the national average" = "red", "Above the national average" = "lightblue", "Same as the national average" = "black")
```
...
medicare_ratings_map %>%
  ...
  geom_polygon(data = us_map, aes(x=long, y=lat, group=group),
                   color="#9EA5A9", fill = "#CED5DA") +
  geom_point(data = subset(medicare_ratings_map, 
                           effectiveness_of_care_national_comparison == "Same as the national average"),
             aes(x = long, y = lat, color = effectiveness_of_care_national_comparison), alpha = .2) +
  geom_point(data = subset(medicare_ratings_map, 
                           effectiveness_of_care_national_comparison == "Above the national average"),
             aes(x = long, y = lat, color = effectiveness_of_care_national_comparison), alpha = .6) +
  geom_point(data = subset(medicare_ratings_map, 
