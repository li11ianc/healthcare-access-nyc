...
```{r hospital-map-test}
# try to make the map interactive
us_map <- st_read("map_data/tl_2017_us_state.shp", stringsAsFactors = FALSE)
datamap <- left_join(hospital_cnt, us_map, by = c("state" = "STUSPS"))
class(datamap)
class(us_map)

glimpse(datamap)

gg_hover <- ggplot(datamap, mapping = aes(fill = hospital_percap, geometry = geometry)) +
  geom_sf_interactive(aes(data_id = states, tooltip = states), color = "white") +
...
hospital_map <- girafe(code = {print(gg_hover)}, height_svg = 6, width_svg = 9)

girafe_options(hospital_map, opts_hover(css = "stroke:black;r:5pt;"))
```
